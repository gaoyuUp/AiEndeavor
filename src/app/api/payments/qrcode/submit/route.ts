...
export async function POST(request: Request) {
  try {
    ...
    await db.$transaction(async (tx) => {
      ...
      if (!payment || payment.orderId !== order.id || payment.provider !== "qrcode") {
        throw new StoreError("电子收款码支付记录不存在", 404);
      }
      if (payment.status === "SUBMITTED") return;
      if (payment.status !== "PENDING" || order.orderStatus !== "PENDING_PAYMENT") {
        throw new StoreError("当前订单不可确认付款", 409);
      }
      ...
      await tx.paymentEvent.create({
        data: {
          ...
          eventKey: `qrcode:submitted:${payment.id}`,
          eventType: "payment.submitted",
          payload: { orderNo: order.orderNo, provider: "qrcode" },
